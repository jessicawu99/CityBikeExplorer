...
GEOCODING_KEY = 'pk.eyJ1IjoiamVzc2ljYWx3dSIsImEiOiJjbGdhMmJjYmIwdXAxM2VwOTNzNzhsZmxjIn0.pyd1X8CXQBA-_Djhbd-0iw' #API key for Geocoding API
...
def create_route():
    # Handling user input from homepage
    city = request.form['city']
    address = request.form['address']
    desired_length = request.form['desired_length']

    # Prompts user if input is incorrect and/or incomplete
    ...
    # Determining whether or not user's city has a bike-share network
    for place in bike_cities:
        if city.lower() in place:
            # User's city HAS a network. Retrieving network and station information
            ...
            # Finding closest station to user
            ...
            # Creating list of end stations in user's desired range
            stns_in_range = []
            for station in stations:
            ...
            # If no stations near desired range, let user know
            if stns_in_range == []:
            ...
            # Program's suggested end station
            choice = stns_in_range[0]

            # Suggesting POI near end station
            ...
            # Generating Google Maps link of route
            ...
            # Creating tree of four more stations of similar distance away and farther (by 1km) distance away
            more_stns = []
            for station in stations:
                rt_length = calculate_distance((station['latitude'], station['longitude']), current_coord)
    ...
    # If no networks in user's city, let user know
    return render_template('nonetwork.html')


@app.route('/viewroutes', methods=['POST'])
def view_route():
    # Handling form input to see if user would like to see additional end points
